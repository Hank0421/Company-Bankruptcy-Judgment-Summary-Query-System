...
# === Gemini API 設定 ===
genai.configure(api_key="AIzaSyAWCFqEi2s977vs-dCHZATNT608sKXb7bk")  # 請填入你的 Gemini API Key
...
def extract_fields_from_html(html):
    ...
    for line in raw_text.splitlines():
        m = re.match(r'^[\s　]*[申聲][\s　]*請[\s　]*人[\s　]*(.+)$', line)
        if m:
...
if results:
    ...
    columns = ["裁判字號", "裁判日期", "申請人", "是否核准", "理由摘要", "是否有抗告", "Gemini摘要", "url"]
